...
import {
	createGame, joinGame, newPlayerPage, resetDice, scriptDice, startGame,
} from '../helpers/game';
...
test('bonus die: V reports stock, Bus shows a reconnect-safe choice and moves by the selection', async ({ browser }) => {
	...
	const roller = await ana.locator('#dice-button').getAttribute('aria-disabled') === 'false'
		? ana
		: berto;
	await expect(roller.locator('#dice-button')).toHaveAttribute('aria-disabled', 'false');
	await scriptDice(2, 3, 5);
	await roller.locator('#dice-button').click();

...
/// <summary>
/// Pins the SHIPPED "The Great Route" journey deck (the dangling-key net in KeyIntegrityTests
...
/// </summary>
public class GreatRoutePackageTests
{
	private static readonly Task<CorroServer.Models.Corro.GameDefinition> Loaded =
		new CorroPackageLoader().LoadAsync(CorroTestPaths.PackageDir("great-route"));
	...
	public async Task The_deck_has_the_classic_106_card_composition()
	{
		...
		var distance200 = deck.Single(c => c.Id == "distance-200");
		Assert.True(distance200.Premium);
		Assert.Equal(2, distance200.MaxPlaysPerHand);

		// Right of way is the multi-shield immunity (red light + speed limit).
		var rightOfWay = deck.Single(c => c.Id == "right-of-way");
		Assert.Equal(new[] { "stop", "speedLimit" }, rightOfWay.ShieldsKinds);
	}
	...
	{
		var path = Path.Combine(CorroTestPaths.PackageDir("great-route"), $"help.{language}.md");
	...
	{
		var credits = File.ReadAllText(Path.Combine(CorroTestPaths.PackageDir("great-route"), "CREDITS.md"));

		Assert.Contains("assets/sounds/low-blow.ogg", credits);

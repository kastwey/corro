...
/// <summary>
/// Pins the SHIPPED "Taller Galáctico" assembly deck (the dangling-key net in
...
/// </summary>
public class TallerGalacticoPackageTests
{
	private static readonly Task<CorroServer.Models.Corro.GameDefinition> Loaded =
		new CorroPackageLoader().LoadAsync(CorroTestPaths.PackageDir("taller-galactico"));
	...
	public async Task The_deck_has_the_68_card_composition()
	{
		...
		Assert.Equal(1, deck.Single(c => c.Id == "modulo-omni").Count);
		Assert.Equal(AssemblyRulebook.Wild, deck.Single(c => c.Id == "modulo-omni").Color);
		Assert.Equal(1, deck.Single(c => c.Id == "averia-omni").Count);

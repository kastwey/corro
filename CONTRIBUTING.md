...
6. **Open the PR** against `main`. It opens pre-filled with the repository's
   [pull request template](.github/pull_request_template.md): fill in the sections that
   apply and delete the rest. Link any related issue. Keep PRs focused — one logical
   change per PR reviews faster.
...
([Spanish](docs/package-authoring.es.md)) before using the format reference.
...
- **Keep themed assets in the package.** Player pieces belong in `assets/tokens/<id>.svg`, optional
  card illustrations in `assets/cards/<card-id>.svg`, optional `artColor: "#RRGGBB"` accents in card
  definitions, and game earcons in `assets/sounds/`. Shared engine

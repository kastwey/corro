...
3. Optional card art belongs in exactly one `assets/cards/<card-id>.svg` (64×64, path geometry only) or strict static 256×256 `.png`. A card may add `artColor: "#RRGGBB"`; missing art uses Corro's neutral fallback. Never put package-specific drawings in engine code.
